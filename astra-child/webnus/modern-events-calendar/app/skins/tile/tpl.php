...
if($this->next_previous_button)
{
    ...
    {
        $navigator_html .= '<div class="mec-previous-month mec-load-month mec-previous-month" data-mec-year="'.date('Y', $_1month_before).'" data-mec-month="'.date('m', $_1month_before).'"><a href="#" class="mec-load-month-link"><i class="mec-sl-angle-left"></i> <span class="displayed-month">'.esc_html($this->main->date_i18n('F', $_1month_before)).'</span></a></div>';
    }
    ...
    {
        $navigator_html .= '<div class="mec-next-month mec-load-month mec-next-month" data-mec-year="'.date('Y', $_1month_after).'" data-mec-month="'.date('m', $_1month_after).'"><a href="#" class="mec-load-month-link"><span class="displayed-month">'.esc_html($this->main->date_i18n('F', $_1month_after)).'</span> <i class="mec-sl-angle-right"></i></a></div>';
    }
}
...
?>
<div id="mec_skin_<?php echo esc_attr($this->id); ?>" class="custom-mec-wrap <?php echo esc_attr($event_colorskin . ' ' . $this->html_class . ' ' . $set_dark); ?>">

    <?php if($this->sf_status) echo MEC_kses::full($this->sf_search_form()); ?>

    <div class="mec-tile hey-iszszi">
        <div class="mec-calendar-topsec">
            <div class="mec-clear">
                <?php if($this->next_previous_button): ?>
                    <!-- removed .mec-skin-tile-month-navigator-container -->
                <div class="mec-skin-tile-month-navigator-container month-navigator-container-custom">
